...
from tsugite_daemon.config import AutoCompactConfig, RuntimeDefaults, load_daemon_config
...
def _make_scheduler(tmp_path, adapter, store, min_turns=1):
    agent_config = RuntimeDefaults(
        workspace_dir=tmp_path,
        agent_file="default",
        auto_compact=AutoCompactConfig(schedule="0 0 * * *", min_turns=min_turns),
    )
    return CompactionScheduler(agent_config, store, adapter), agent_config
...
class TestAgentConfigAutoCompact:
    def test_none_by_default(self):
        cfg = RuntimeDefaults(workspace_dir=Path("/tmp"), agent_file="default")
...
class TestConfigParsing:
    def test_load_auto_compact_from_yaml(self, tmp_path):
        config_data = {
            "default_workspace_dir": str(tmp_path),
            "default_agent_file": "assistant.md",
            "auto_compact": {
                "schedule": "0 0 * * *",
                "min_turns": 3,
            },
        ...
        assert config.runtime.auto_compact is not None
        assert config.runtime.auto_compact.schedule == "0 0 * * *"
        assert config.runtime.auto_compact.min_turns == 3
...
class TestCompactionScheduler:
    def test_check_agent_skips_low_turns(self, tmp_path):
        ...
        asyncio.run(scheduler._check_sessions())
    ...
    def test_check_agent_no_adapter(self, tmp_path):
        ...
        scheduler = CompactionScheduler(agent_config, store, None)
    ...
    def test_compaction_failure_handled(self, tmp_path):
        ...
        assert not store.is_compacting("user1")
...
class TestCompactionSchedulerTimezone:
    def test_compute_next_fire_time_uses_agent_timezone(self, tmp_path):
        ...
        fire = scheduler._compute_next_fire_time(now)
...
class TestSessionStoreListInteractive:
    def test_list_interactive(self, tmp_path):
        store = _make_session_store(tmp_path)
        store.get_or_create_interactive("user1")
        store.get_or_create_interactive("user2")
        store.get_or_create_interactive("user1")

        sessions = store.list_interactive()
...
class TestMidTurnGuard:
    def test_check_agent_skips_session_with_turn_in_flight(self, tmp_path):
        ...
        store = _make_session_store(tmp_path)
        s = Session(id="busy", source=SessionSource.INTERACTIVE.value, user_id="u")
        s.message_count = 10
