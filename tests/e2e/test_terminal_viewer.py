...
def _stub_terminals_api(page, terminals=None, stream_chunks=None, stream_state=None):
    """Intercept /api/terminals + the SSE stream with deterministic fixtures.

    `terminals` is the list returned by GET /api/terminals.
    `stream_chunks` is a sequence of output strings the SSE stream emits.
    ...
    page.route("**/api/terminals/*/restart", lambda route: route.fulfill(status=204, body=""))
...
def test_xterm_host_mounted_after_selection(authenticated_page):
    ...
    has_xterm = page.evaluate(
        "() => !!document.querySelector('[data-xterm-host] .xterm')"
    )
...
def test_terminal_section_uses_peach_label_color(authenticated_page):
    ...
    peach = page.evaluate(
        "() => getComputedStyle(document.body).getPropertyValue('--peach').trim()"
    )
    color = page.evaluate(
    ...
    overlay0 = page.evaluate(
        "() => getComputedStyle(document.body).getPropertyValue('--overlay0').trim()"
    )
