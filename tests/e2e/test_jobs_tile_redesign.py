...
def test_redesigned_tile_renders_all_seven_states(authenticated_page, e2e_adapter, e2e_tmp):
    ...
    with patch("tsugite.daemon.adapters.http.get_history_dir", return_value=history_dir):
        ...
        assert tiles.count() == len(JOB_STATES), (
            f"Expected {len(JOB_STATES)} tiles, got {tiles.count()}"
        )
        states_rendered = {tiles.nth(i).get_attribute("data-state") for i in range(tiles.count())}
        assert states_rendered == set(JOB_STATES), (
            f"All seven states must render; got {sorted(states_rendered)}"
        )
...
def test_attempts_expander_shows_when_multiple(authenticated_page, e2e_adapter, e2e_tmp):
    ...
    storage.record(
        ...
        verify_attempts=2,
        attempts=[
            {"index": 0, "kind": "initial", "worker_session_id": "w1", "verifier_session_id": "v1", "verifier_pass": False},
            {"index": 1, "kind": "retry", "worker_session_id": "w2", "verifier_session_id": "v2", "verifier_pass": False},
            {"index": 2, "kind": "retry", "worker_session_id": "w3", "verifier_session_id": "v3", "verifier_pass": False},
        ],
    )
...
def test_retry_with_hint_dialog_opens_and_submits(authenticated_page, e2e_adapter, e2e_tmp, base_url):
    ...
    with patch("tsugite.daemon.adapters.http.get_history_dir", return_value=history_dir):
        ...
        # Type a hint.
        page.locator(".modal-backdrop.open textarea#retry-hint-text").fill("try again with a smaller diff")
        # Submit.
        page.locator(".modal-backdrop.open button.btn-primary").click()
