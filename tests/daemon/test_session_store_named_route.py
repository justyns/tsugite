...
def test_find_named_session_returns_none_when_absent(store):
    assert store.find_named_session("u1", "discord") is None


def test_get_or_create_named_session_creates_with_metadata(store):
    s = store.get_or_create_named_session("u1", "discord")
    assert s.metadata.get("session_name") == "discord"
    assert s.user_id == "u1"


def test_get_or_create_named_session_idempotent(store):
    s1 = store.get_or_create_named_session("u1", "discord")
    s2 = store.get_or_create_named_session("u1", "discord")
    assert s1.id == s2.id


def test_named_session_isolated_per_user(store):
    s1 = store.get_or_create_named_session("u1", "discord")
    s2 = store.get_or_create_named_session("u2", "discord")
    assert s1.id != s2.id


def test_named_session_isolated_per_name(store):
    s1 = store.get_or_create_named_session("u1", "discord")
    s2 = store.get_or_create_named_session("u1", "slack")
    assert s1.id != s2.id
...
def test_find_named_session_skips_superseded(store):
    ...
    found = store.find_named_session("u1", "discord")
