...
def test_round_trip_via_reload(store_path):
    ...
    s1.update_state(t.id, TerminalState.SUCCEEDED.value)
    ...
    assert loaded.state == TerminalState.SUCCEEDED.value
...
def test_load_reconciles_stale_active_records_to_stream_lost(store_path):
    """Records persisted as starting/running belong to a dead daemon process -
    the new PtyManager has no proc for them, so kill/restart would wedge. The
    loader must mark them stream_lost (resolved) instead."""
    s1 = TerminalSessionStore(store_path)
    running = s1.add(_make_terminal())
    s1.update_state(running.id, TerminalState.RUNNING.value)
    starting = s1.add(_make_terminal(cmd="sleep 1"))
    done = s1.add(_make_terminal(cmd="true"))
    s1.update_state(done.id, TerminalState.RUNNING.value)
    s1.update_state(done.id, TerminalState.SUCCEEDED.value)

    # Simulated daemon restart.
    s2 = TerminalSessionStore(store_path)
    for tid in (running.id, starting.id):
        t = s2.get(tid)
        assert t.state == TerminalState.STREAM_LOST.value
        assert t.resolved_at
    assert s2.get(done.id).state == TerminalState.SUCCEEDED.value

    # The reconciliation must be persisted, not just in-memory.
    s3 = TerminalSessionStore(store_path)
    assert s3.get(running.id).state == TerminalState.STREAM_LOST.value
