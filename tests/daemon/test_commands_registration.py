...
@pytest.mark.parametrize("cmd", list(get_commands().values()), ids=lambda c: c.name)
def test_required_params_precede_optional(cmd):
    """The Discord adapter builds an inspect.Signature from CommandParam order,
    so required params (no default) must come before any required=False params
    (which get default=None). Otherwise Python raises 'non-default argument
    follows default argument' at bot startup. Caught a regression where /job's
    optional session_id sat between required user_id and required prompt.
    """
    seen_optional = False
    for p in cmd.params:
        if not p.required:
            ...
            pytest.fail(
                f"Command '/{cmd.name}' has required param '{p.name}' after an "
                f"optional one. Discord signature construction will fail at bot "
                f"startup. Reorder so all required params come first."
            )
