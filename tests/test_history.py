...
class TestStorage:
    ...
    def test_save_and_load_conversation(self, tmp_path, monkeypatch):
        ...
        from tsugite.history import ConversationMetadata, Turn
        ...
        metadata = ConversationMetadata(
            ...
            created_at=datetime(2025, 10, 24, 10, 30, 0, tzinfo=tz.utc),
        )
        save_turn_to_history(conv_id, metadata)

        # Save turn
        turn = Turn(
            timestamp=datetime(2025, 10, 24, 10, 30, 5, tzinfo=tz.utc),
            user="Hello",
            assistant="Hi there!",
            tools=[],
            tokens=50,
    ...
    def test_save_with_malformed_line(self, tmp_path, monkeypatch):
        ...
        # Write a valid metadata line, malformed line, and valid turn line

        conv_file.write_text(
            '{"type": "metadata", "id": "test", "agent": "test", "model": "test", "machine": "test", "created_at": "2025-10-24T10:00:00+00:00"}\n'
            + "not valid json\n"
            + '{"type": "turn", "timestamp": "2025-10-24T10:00:00+00:00", "user": "test", "assistant": "test"}'
        )
...
class TestIndex:
    ...
    def test_update_and_query_index(self, tmp_path, monkeypatch):
        ...
        from tsugite.history import IndexEntry

        metadata = IndexEntry(
            agent="chat_assistant",
            model="openai:gpt-4o",
            machine="laptop",
            created_at=datetime(2025, 10, 24, 10, 0, 0, tzinfo=tz.utc),
            updated_at=datetime(2025, 10, 24, 10, 30, 0, tzinfo=tz.utc),
            turn_count=5,
            total_tokens=1000,
            total_cost=0.05,
        )
    ...
    def test_rebuild_index(self, tmp_path, monkeypatch):
        ...
        save_turn_to_history(
            conv_id,
            Turn(
                timestamp=datetime(2025, 10, 24, 10, 1, 0, tzinfo=tz.utc),
                user="test",
                assistant="test",
                tokens=100,
                cost=0.01,
    ...
    def test_format_conversation_for_display(self, tmp_path, monkeypatch):
        ...
        turns = [
            ConversationMetadata(
                ...
                model="gpt-4o",
                machine="laptop",
                created_at=datetime.fromisoformat("2025-10-24T10:00:00+00:00"),
            ),
            Turn(
                timestamp=datetime.fromisoformat("2025-10-24T10:00:05+00:00"),
                user="Hello",
                assistant="Hi!",
                tools=["search"],
                tokens=50,
