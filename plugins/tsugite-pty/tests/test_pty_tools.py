...
def test_pty_send_keys_ctrl_c_interrupts(runtime):
    ...
    # Sending Ctrl+C kills sleep; the shell reports the signal (non-zero exit -> FAILED, or
    # CANCELLED/SUCCEEDED). Delivery via the PTY line discipline can race a loaded CI runner,
    # and TIOCSCTTY is best-effort across kernels, so resend periodically until the session
    # leaves the running state. The poll returns as soon as it does; the ceiling is generous
    # only to absorb a slow runner.
    sink = {
        TerminalState.FAILED.value,
        TerminalState.CANCELLED.value,
        TerminalState.SUCCEEDED.value,
    ...
    while time.monotonic() < deadline:
        final = store.get(tid)
        if final and final.state in sink:
            break
        if time.monotonic() - last_resend > 1.0:
            try:
                terminal_tools.pty_send_keys(tid, "\x03", enter=False)
            except Exception:
                pass  # child may have just exited; the next poll will see the sink state
            last_resend = time.monotonic()
        time.sleep(0.05)

    assert final is not None, "terminal session disappeared"
    assert final.state in sink, f"Ctrl+C did not interrupt; final state was {final.state!r}"
