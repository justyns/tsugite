...
import pytest
from onlyoffice_helpers import PUBLIC_BASE_URL, SECRET_NAME, SERVER_URL
...
def test_absent_config_returns_none():
    ...
    assert _create({**VALID_CONFIG, "enabled": False}) is None
...
def test_a_config_nobody_turned_on_is_not_read_at_all():
    """The switch is above the parse, so a block left half-written under `enabled: false` stays
